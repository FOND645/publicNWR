...
db.serialize(() => {
        ...
        db.run(
            ...
            (error) => {
                ...
                if (error) {
                    ...
                    console.log(
                        `Добавлен материал ID:${materialID}, name:${name}`
                    );
                }
            }
        );
    ...
    JSONdatabse.devices.forEach((device) => {
        ...
        total++;
        db.run(
            'INSERT INTO devices (id, name, decimal) VALUES (?, ?, ?)',
            [deviceID, name, decimal],
            (error) => {
                complite++;
                console.log(`${complite} из ${total}`);
                if (error) {
                    ...
                    console.log(
                        `Добавлено устройство ID:${deviceID}, name:${name}`
                    );
                }
            }
        );
        ...
        total++;
        db.run(
            'INSERT INTO blocks (id, name, decimal, is_leading) VALUES (?, ?, ?, ?)',
            [deviceBlockID, name, decimal, 1],
            (error) => {
                complite++;
                console.log(`${complite} из ${total}`);
                if (error) {
                    console.log(
                        'Ошибка при добавлении устройства в блоки',
                        error
                    );
                } else {
                    console.log(
                        `Добавлено устройство-блок ID:${deviceBlockID}, name:${name}`
                    );
                }
            }
        );
        ...
        total++;
        db.run(
            'INSERT INTO blocks_in_devices (device_id, block_id) VALUES (?, ?)',
            [deviceID, deviceBlockID],
            (error) => {
                complite++;
                console.log(`${complite} из ${total}`);
                if (error) {
                    console.log(
                        'Ошибка при создании связи Устройство - устройство в блоках',
                        error
                    );
                } else {
                    console.log(
                        `Добавлено связь Устройство - устройство в блоках ID:${deviceBlockID}, name:${name}`
                    );
                }
            }
        );
            ...
            if (blocksIDs.hasOwnProperty(key)) {
                ...
                db.run(
                    ...
                    (error) => {
                        ...
                        if (error) {
                            console.log(
                                'Ошибка при добавлении существующего блока',
                                error
                            );
                        } else {
                            console.log(
                                `Добавлена связь: ${deviceID} - ${blockID}`
                            );
                        }
                    }
                );
                ...
                total++;
                db.run(
                    'INSERT INTO blocks (id, name, decimal) VALUES (?, ?, ?)',
                    [blockID, name, decimal],
                    (error) => {
                        complite++;
                        console.log(`${complite} из ${total}`);
                        if (error) {
                            console.log(
                                'Ошибка при добавлении нового блока',
                                error
                            );
                        } else {
                            console.log(
                                `Добавлен блок ID:${blockID}, name:${name}`
                            );
                        }
                    }
                );
    ...
    db.run(
        ...
        (error) => {
            ...
            if (error) {
                console.log(
                    'Ошибка при создании таблицы связи ДЕЙСТВИЕ - МАТЕРИАЛЫ',
                    error
                );
            } else {
    ...
    JSONdatabse.defects.forEach((Defect) => {
        ...
        total++;
        db.run(
            `INSERT INTO defects (id, description, defect, solution, block_id) VALUES (?, ?, ?, ?, ?)`,
            [defectID, description, defect, solution, blockID],
            (error) => {
                complite++;
                console.log(`${complite} из ${total}`);
                if (error) {
                    ...
                    console.log(
                        `Добавлена неисправность ID:${defectID}, name:${description}`
                    );
                }
            }
        );
        ...
        actions.forEach((Action) => {
            ...
            total++;
            db.run(
                `INSERT INTO actions ("id", "action", "index", "defect_id") VALUES (?, ?, ?, ?)`,
                [actionID, action, index, defectID],
                (error) => {
                    complite++;
                    console.log(`${complite} из ${total}`);
                    if (error) {
                        ...
                        console.log(
                            `Добавлено действие ID:${actionID}, name:${action}`
                        );
                    }
                }
            );
            ...
            materials.forEach((Material) => {
                ...
                db.run(
                    ...
                    (error) => {
                        ...
                        if (error) {
                            console.log(
                                'Ошибка при связывании ДЕЙСТВИЕ - МАТЕРИАЛ',
                                error
                            );
                        } else {
                            console.log(
                                `Добавлена связь ${actionID} - ${materialID}`
                            );
                        }
                    }
                );
    ...
    db.run(
        ...
        (err) => {
            ...
            if (err) {
                console.log(
                    `Ошибка создания таблицы связи БЛОК в ремонте - НЕИСПРАВНОСТИ`
                );
            } else {
                console.log(
                    `Создана таблица связи БЛОК в ремонте - НЕИСПРАВНОСТИ`
                );
            }
        }
    );
    ...
    organiztionsIDs.forEach((Organization, id) => {
        ...
        total++;
        db.run(
            'INSERT INTO organiztions ("id", "name", "city") VALUES (?, ?, ?)',
            [id, name, city],
            (err) => {
                complite++;
                console.log(`${complite} из ${total}`);
                if (err) {
                    console.log(
                        `Ошибка добавления организации ${id} ${name} ${city}`,
                        err
                    );
                } else {
    ...
    JSONdatabse.contracts.forEach((Contract) => {
        ...
        total++;
        db.run(
            `INSERT INTO contracts ("id", "number", "date", "organiztion_id") VALUES (?, ?, ?, ?)`,
            [contractID, contractNumber, contractDate, organiztionID],
            (err) => {
                complite++;
                console.log(`${complite} из ${total}`);
                if (err) {
                    console.log(
                        `Ошибка добавления контракта ${contractID} ${contractNumber} ${organiztionID}`,
                        err
                    );
                } else {
                    console.log(
                        `Добавлен контракт ${contractID} ${contractNumber} ${organiztionID}`
                    );
                }
            }
        );
        repairBase.forEach((RepairDevice) => {
            ...
            total++;
            db.run(
                `INSERT INTO repair_devices ("id", "contract_id", "device_id", "create_time", "change_time", "repair_number", "serial_number", "divided") VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
                [
                    repairDeviceID,
                    contractID,
                    deviceID,
                ...
                (err) => {
                    ...
                    if (err) {
                        console.log(
                            `Ошибка добавления устройства ремонта ${repairDeviceID}, ${contractID}, ${deviceID}, ${createTime}, ${changeTime}, ${repairNumber}, ${serialNumber}`,
                            err
                        );
                    } else {
                        console.log(
                            `Добавлено устройство ремонта ${repairNumber} ${repairDevicesIDs} ${deviceID} ${serialNumber}`
                        );
                    }
                }
            );
            ...
            RepairDevice.notes.forEach((Note) => {
                ...
                noteID++;
                db.run(
                    `INSERT INTO repair_notes ("id", "text", "date", "repair_device_id") VALUES (?, ?, ?, ?)`,
                    [noteID, text, date, repairDeviceID],
                    (result: [], error: Error | null) => {
                        complite++;
                        console.log(`${complite} из ${total}`);
                        if (error) {
                            console.log(
                                `Ошибка добавления примечание к ремонту`,
                                noteID,
                                text,
                                date,
                                repairDeviceID
                            ...
                            console.log(
                                `Добавлено примечание к ремонту`,
                                noteID,
                                text,
                                date,
                                repairDeviceID
            ...
            subDevices.forEach((Block) => {
                ...
                total++;
                db.run(
                    `INSERT INTO repair_blocks ("id", "block_id", "serial_number", "count", "repair_device_id") VALUES (?, ?, ?, ?, ?)`,
                    [
                        repairBlockID,
                        blockID,
                        serialNumber,
                    ...
                    (err) => {
                        ...
                        if (err) {
                            console.log(
                                `Ошибка добавления блока ремонта ${repairBlockID}, ${blockID}, ${serialNumber}, ${count}`,
                                err
                            );
                        } else {
                            console.log(
                                `Добавлено блока ремонта ${blockID} ${serialNumber}`
                            );
                        }
                    }
                );
                ...
                defects.forEach((Defect) => {
                    ...
                    total++;
                    db.run(
                        `INSERT INTO defects_in_repair ("repair_blocks_id", "defect_id") VALUES (?, ?)`,
                        ...
                        (err) => {
                            ...
                            if (err) {
                                console.log(
                                    `Ошибка добавления связи БЛОК ${repairBlockID} - НЕИСПРАВНОСТЬ ${defectID}`,
                                    err
                                );
                            } else {
                                console.log(
                                    `Добавлена связь БЛОК ${repairBlockID} - НЕИСПРАВНОСТЬ ${defectID}`
                                );
                            }
                        }
                    );
